...
public class OrderServiceTest {
    ...
    public void 주문취소() throws Exception {
        ...
        //when (내가 테스트 하고 싶은 부분)
        orderService.cancelOrder(orderId);

        //then (검증)
        Order getOrder = orderRepository.findOne(orderId);
        Assertions.assertEquals(OrderStatus.CANCLE, getOrder.getStatus(), "주문 취소시 상태는 CANCLE이다.");
        Assertions.assertEquals(10, item.getStockQuantity(), "주문이 취소된 상품은 그만큼 재고가 증가 해야 한다.");
    }
